...
def reduce_mem_usage(df):
    ...
    print(
        'Mem. usage decrease to {:5.2f}Mb ({:.1f}% reduction)'.format(end_mem, 100 * (start_mem - end_mem) / start_mem))
    return df
...
def xgb_cv(max_depth, min_child_weight, subsample, colsample_bytree, data, target):
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(data, target)):
        ...
        clf = xgb.train(param, trn_data, 10000, evals=[(trn_data, 'train'), (val_data, 'valid')], verbose_eval=200,
                        early_stopping_rounds=600)
